...
USER_DB_FILE = os.path.join(SAVE_FOLDER, "system_users.json")

# ====================================================================
# 🔒 DYNAMIC ACCOUNT PROFILE PRIVILEGE ENGINE
# ====================================================================
def load_dynamic_system_users():
    if os.path.exists(USER_DB_FILE):
        try:
            with open(USER_DB_FILE, "r", encoding="utf-8") as f: return json.load(f)
        except Exception: pass
    default_users = {
        "publisher": {"password": "pub123", "role": "Publisher"},
        "admin": {"password": "admin123", "role": "Administrator"},
        "reporter": {"password": "rep123", "role": "Reporter"}
    }
    with open(USER_DB_FILE, "w", encoding="utf-8") as f:
        json.dump(default_users, f, ensure_ascii=False, indent=4)
    return default_users

def save_dynamic_system_users(users_dict):
    with open(USER_DB_FILE, "w", encoding="utf-8") as f:
        json.dump(users_dict, f, ensure_ascii=False, indent=4)

SYSTEM_USERS = load_dynamic_system_users()

if "authenticated" not in st.session_state: st.session_state["authenticated"] = False
if "current_user" not in st.session_state: st.session_state["current_user"] = None
if "current_role" not in st.session_state: st.session_state["current_role"] = None
if 'active_image_stream' not in st.session_state: st.session_state['active_image_stream'] = None
...
saved_files = [f for f in os.listdir(SAVE_FOLDER) if f.endswith('.json') and f != "system_users.json"]
saved_files.sort(reverse=True)

all_articles = {}
pending_notifications_count = 0  # Tracker to count pending articles for notifications
...
# ====================================================================
# 🎨 BRAND SYSTEM GLOBAL STYLING OVERLAYS
# ====================================================================
st.markdown(
    """
    <style>
        ...
        [data-testid="stHeader"] { background-color: transparent !important; background: none !important; }
        .stAppDeployButton, [data-testid="stDeploymentButton"] { display: none !important; visibility: hidden !important; }
        ...
        .single-epaper-button-card {
            display: inline-block !important; background-color: #ffffff !important; border: 2px solid #0d47a1 !important; border-radius: 6px !important; padding: 6px 12px !important; cursor: pointer !important; text-decoration: none !important; transition: background-color 0.2s, transform 0.1s; margin-bottom: 20px !important; width: 100% !important; max-width: 310px !important;
        }
...
def render_unified_branding_masthead():
    if os.path.exists("official_logo.png"): st.image("official_logo.png", width=320)
    else: st.markdown("<h1 style='color:#0d47a1; font-size:48px; margin:0;'>డెక్కన్</h1><p style='color:#d32f2f; margin:0;'>ప్రజలకు, అధికారులకు మధ్య వారధి</p>", unsafe_allow_html=True)
...
if is_admin_url:
    ...
    if not st.session_state["authenticated"]:
        ...
        user_input = st.text_input("Username:", placeholder="Enter user ID...", key="adm_user_in")
        pass_input = st.text_input("Password:", type="password", placeholder="Enter secure key...", key="adm_pass_in")
        
        if st.button("🔒 Unlock Dashboard", use_container_width=True):
            if user_input in SYSTEM_USERS and pass_input == SYSTEM_USERS[user_input]["password"]:
                st.session_state["authenticated"] = True
                st.session_state["current_user"] = user_input
                st.session_state["current_role"] = SYSTEM_USERS[user_input]["role"]
                st.rerun()
            else: st.error("❌ Invalid Username or Password verification values.")
    else:
        st.sidebar.success(f"🟢 User: {st.session_state['current_user']} ({st.session_state['current_role']})")
        ...
        # UPGRADE: Flash alert notification bar on the Publisher & Admin workspace panels if pending live entries exist
        if st.session_state["current_role"] in ["Publisher", "Administrator"] and pending_notifications_count > 0:
            st.warning(f"🔔 Notification: There are **{pending_notifications_count} live articles** currently pending review and editorial approval confirmation.")

        if st.session_state["current_role"] == "Publisher":
            st.sidebar.markdown("---")
            with st.sidebar.expander("👥 User Account Provisioning Console", expanded=False):
                new_username = st.text_input("New Username:", placeholder="e.g. reporter_kumar", key="new_u_field").strip().lower()
                new_password = st.text_input("Set Password:", placeholder="e.g. securePass1", key="new_p_field").strip()
                new_role = st.selectbox("Assign Privilege Level:", ["Reporter", "Administrator", "Publisher"])
                if st.button("➕ Provision Account", use_container_width=True):
                    if not new_username or not new_password: st.error("Fields cannot be blank!")
                    else:
                        SYSTEM_USERS[new_username] = {"password": new_password, "role": new_role}
                        save_dynamic_system_users(SYSTEM_USERS)
                        st.success(f"🎉 Account '{new_username}' added!")
                        st.rerun()
                st.write("---")
                for u_name, u_info in list(SYSTEM_USERS.items()):
                    if u_name == "publisher": continue
                    col_u_txt, col_u_del = st.columns([0.7, 0.3])
                    col_u_txt.caption(f"👤 **{u_name}** ({u_info['role']})<br>🔑 Pass: `{u_info['password']}`", unsafe_allow_html=True)
                    if col_u_del.button("❌", key=f"del_user_row_{u_name}"):
                        del SYSTEM_USERS[u_name]
                        save_dynamic_system_users(SYSTEM_USERS)
                        st.rerun()
                        
        st.sidebar.markdown("---")
        st.sidebar.subheader("📁 పాత వార్తలు (History Manager)")
        admin_search_query = st.sidebar.text_input("🔍 వెతకండి (Search past article title):", placeholder="Type title keywords...", key="adm_history_search_input").strip()
        ...
        if is_edit_mode:
            ...
            default_views = loaded_data.get("views", 0)
            default_author = loaded_data.get("author", "Unknown Author")
            default_status = loaded_data.get("status", "Pending")
            page_header = f"📝 Editing Past Article: {selected_headline} (by {default_author})"
            ...
            default_image_log = ""
            default_views = 0
            default_author = st.session_state["current_user"]
            default_status = "Pending"
            page_header = "➕ Create and Format a New Article"
            button_label = "🚀 Publish and Save New Article to Archives"

        # Check privilege constraints and approval status state flag
        is_reporter = st.session_state["current_role"] == "Reporter"
        is_locked_for_reporter = is_reporter and default_status == "Approved"

        if is_locked_for_reporter:
            st.error(f"🔒 This live story has been **Approved** by the executive editorial board. Your access token is now locked to protect compliance history records.")
            st.markdown(f"**Headline:** {default_headline}")
            st.markdown(f"**Content:** {default_body}")
        else:
            st.markdown(f"### {page_header}")
            f_suffix = active_file.replace('.json', '') if active_file else 'new'
            
            headline_input = st.text_input(label="Main Headline:", value=default_headline, key=f"main_head_form_{f_suffix}")
            sub_header_input = st.text_input(label="Sub-Header Option (Optional):", value=default_sub_header, key=f"sub_head_form_{f_suffix}")
            ...
            if st.session_state["current_role"] in ["Publisher", "Administrator"] and is_edit_mode:
                status_input = st.selectbox(
                    label="🛡️ Editorial Review & Approval Status State Flag:",
                    options=["Pending", "Approved"],
                    index=["Pending", "Approved"].index(default_status),
                    key=f"status_form_{f_suffix}"
                )
            else:
                status_input = default_status
                if is_edit_mode:
                    st.info(f"📋 Current Status: **{default_status}** (This news is already LIVE on the home feed. Waiting for Admin/Publisher approval review to lock editing access keys)")
...
# --------------------------------------------------------------------
    ...
    if current_viewing_file is not None:
        ...
        current_hits = full_art.get("views", 0) + 1
        full_art["views"] = current_hits
        
        with open(os.path.join(SAVE_FOLDER, current_viewing_file), "w", encoding="utf-8") as f:
            json.dump(full_art, f, ensure_ascii=False, indent=4)
        ...
        with col_right_trending:
            ...
            if all_articles:
                for side_headline, side_file in list(all_articles.items())[:6]:
                    ...
                    sub_html = f'<p style="font-size:11px; color:#666; margin-top:3px; margin-bottom:0; line-height:1.2;">{s_sub_val[:40]}... <br>👁️ {s_views} Views</p>' if s_sub_val else f'<p style="font-size:11px; color:#666; margin-top:3px; margin-bottom:0;">👁️ {s_views} Views</p>'
            ...
            else: st.caption("No trending headlines recorded in archives yet.")
